...
### 2.2 관측 (2026-09-23) — 널리 인용되는 공개 사이트가 낮게 나온다

§2.1 은 낮은 중앙값을 "설계 의도(1층만·품질 무시로 부풀지 않게)가 작동한다" 로 읽었다.
그 해석이 맞으려면 **정말 준비된 사이트는 높게 나와야 한다.** 그래서 양성 대조군을 쟀다.

| 대상 | 자동 티어 ARS | 권위(P4) 만점 가정 |
|---|---|---|
| 한국어 위키백과 | **38.6** | 50.8 |
| MDN (developer.mozilla.org) | **37.5** | 50.0 |
| web.dev | **41.7** | 53.4 |

널리 인용되는 사이트가 §2.1 코호트 중앙값(42.1) 아래다. 권위를 만점으로 가정해도 60 을
넘지 못한다. 가르는 것은 구조화 데이터(P3 전체) · llms.txt · AI 봇 개별 지시 · 질문형
헤딩 같은 **권장 관행의 이행 여부**다 — MDN 의 P3 기여는 0 이다.

따라서 **§2.1 의 해석을 유보한다.** 낮은 점수가 "부풀림을 막았다" 인지 "인용과 무관한
관행을 잰다" 인지 이 데이터로는 가를 수 없고, 후자를 지지하는 반례가 생겼다.

> ⚠️ 사전 예측 없는 사후 분석이고 표본이 셋이다. 이 대상들의 인용 빈도도 우리가 직접
> 재지 않았다. **가설의 근거이지 검정이 아니다** — 검정은 `plane-a-visibility.md` §6.4
> 의 조건을 갖춘 코호트로 한다.

> 🔎 참조 구현의 공시: 이 결과를 받아 자동 티어 리포트는 ARS 를 **"권장 관행 이행도"**
> 로 부르고, 인용 예측력이 검증되지 않았다는 제한을 점수 옆에 붙인다.

출처: `pilot/20260923-validity/`(비공개 저장소). 영어 대상은 한국어 전용 검출 항목
(`P2-08`)의 불공정분을 빼도 1.5~2점 차이라 결론이 바뀌지 않는다.
